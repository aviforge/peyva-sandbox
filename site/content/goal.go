...
- Standard library only. No frameworks, no brokers, no third-party libraries.
  Two things are exempt: the runner, a script you are given rather than build,
  and one database driver, where the language has no SQLite of its own. Name
  the driver you chose. Nothing else is exempt, and never a second driver.
