...
var Chapter16 = ChapterContent{
	...
	Concepts: []ConceptItem{
		...
		{Term: "CAP Theorem", Description: "When the network splits, a system can keep consistency or availability, not both. The choice only binds while the split lasts."},
	},
	...
	BuildIt: BuildIt{
		...
		Prompts: []Prompt{
			{Label: "Decide", Thinking: true, Text: `A payments system keeps balances in a primary copy that replicates asynchronously to a second copy. I need to decide what it does when the two can't reach each other.
...
Done when I have three real options, a reason each rejected one was rejected, and a recommendation with the condition that would overturn it.`},
			{Label: "Build", Text: `The Vault's primary copy replicates asynchronously to a second copy, and you recommended a strategy for what the Teller does when the two cannot reach each other.
...
Done when disconnecting the second copy makes payments fail loudly while enquiries still succeed, and reconnecting restores payments with no restart.`},
			{Label: "Portal", Portal: true, Text: `When the Vault's copies disagree, the balance the Portal shows may be behind.
